 ...
 */
type XwcAddr struct {
	Addr string //xwc main chain address string
}

// using BIP44 to manage xwc address
...
func getXwcAddressByWif(wif string, version uint32) (addr string, err error) {
	addr = ""
...
func GetNewPrivate() (privWif string, pubWif string, addr string, err error) {
	...
	pubWif = "XWC" + base58.Encode(pubByte)
	addr, err = getXwcAddressByWif(privWif, VersionNormalAddr)
	if err != nil {
		return
	}
	return
...
func GetAddress(seed []byte, nettype string, account uint32, addrIndex uint32, version uint32) (string, string, error) {
	...
	mastkey, err := getMasterkey(seed, true) //xwc using btc mainchain cfg
	if err != nil {
...
func DerivePubkey(wif string) (pub string, err error) {
	...
	return "XWC" + base58.Encode(buf), nil
}
