...
// SignAddress sign address to bind  to xwc chain
...
// convert der sign to xwc signature
func DerSignToXwcSign(sig string, recid int) ([]byte, error) {
	bsig, err := common.ConvertDerSig(sig)
	if err != nil {
		return nil, err
	}

	var xwcsig []byte = make([]byte, 65)
	// bsig = r + s
	xwcsig[0] = byte(27 + 4 + int(recid))
	copy(xwcsig[1:], bsig[:])
