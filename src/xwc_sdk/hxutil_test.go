...
var (
	//xwcuri      = "http://192.168.1.220/server/process"
	xwcuri     = "http://192.168.1.220/server/process"
	walletURI = "http://192.168.1.122:10046" // broadcast wallet rpc api
	// zhengqinpeng wid
	xwcwalletid = "dcc02a08142e230814e108053eb40c6180f908da"
	//myxwcaddr   = "XWCNhNthqhgkEfPjzhLQ3cWnNEpEjpjtvmKzw"
	//myhcaddr   = "TsTaA9DT2z24Wg4U31QLeSvcg1agu6e4d88"
	// zhengqinpeng address
	myxwcaddr = "" // "XWCNWjKv1PUbZ6dgVoerkohdoCHRck9LAZh3Y"
	// myxwcaddr = "XWCNXPFrzwsTo7wT5QceDLEjpd2VZ8BZR3UCY"  // just for bind usdt
	// wifxwc = "5KZTntQJ9AAARnuKUzgUG4MHiPPVU7FuMq4d3WNH4rhasDxRa4a" // for bind usdt

	myltcaddr = "LP2JKjy9WmSygMdoe2CzEHabXrPSPXspNF" // "LQz1gokPokkj6hK3dHt1JphZbPh5G9KpbV"
	dstxwcaddr = "XWCNL3kJ4prkHUsHsnwW4HGSDUfYxncWcfgDn"

	mycointype = "XWC"                                  //  "HC(XWC)"
...
func init() {
	resp, err := GetAddress(xwcreq, "XWC")
	if err != nil {
		panic(err.Error())
	}
	// fmt.Printf("derived xwc address is %v\n", resp.Address)
	myxwcaddr = resp.Address
}

func testXWCAddress(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		req.Index = i
		resp, err := GetAddress(&req, "XWC")
		assert.Nil(t, err)
		fmt.Printf("XWC index %d Address: %v\n", i, resp.Address)
	}
}

func testXWCPubkey(t *testing.T) {
	var (
		mnemonic = "beyond stage sleep clip because twist token leaf atom beauty genius food business side grid unable middle armed observe pair crouch tonight away coconut"
		req      AddrReq
	)
	...
	assert.Nil(t, err)
	fmt.Println(pk)
	assert.Equal(t, "XWC5cj54KW1TDK94GCcrSovYDUUYQ1FgCbECnG2CaEv5ub7Vgxwc5N", pk)
}
...
func testUnbind(t *testing.T, wallet bool) {
	ms1 := makeheader("xwcbinding_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm["origAddr"] = myhcaddr
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = mycointype // "HC(XWC)"
	buf1 := doReq(t, ms1)
	fmt.Println("bind response:", string(buf1))

	xwif, err := DumpPrivKey(xwcreq, "HC")
	assert.Nil(t, err)
	_ = xwif

	content := map[string]string{
		"origAddr":    myhcaddr,
		fieldCoinAddr: myxwcaddr,
		"coinType":    mycointype, //"HC(XWC)",
	...
	hex, err := XWCTransaction(xwcreq, XWCUbind, buf1, cbuf)
	assert.Nil(t, err)
...
func testBindBTCUSDT(t *testing.T, wallet bool) {
	bindAddress(t, cointypebtc, myxwcaddr, coinaddrbtc, wifbtc, xwcwalletid, true)
	bindAddress(t, cointypeusdt, myxwcaddr, coinaddrusdt, wifusdt, xwcwalletid, true)
}

func bindAddress(t *testing.T, cointype string, xwcaddr, coinaddr string, wif, walletid string, wallet bool) {
	ms1 := makeheader("xwcbinding_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm["origAddr"] = coinaddr
	bm[fieldCoinAddr] = xwcaddr
	...
	hex, err := XWCTransaction(xwcreq, "bind", buf1, cbuf)
	assert.Nil(t, err)

	ms2 := makeheader("xwcbinding_send", xwcwalletid)
...
func testXWCBind(t *testing.T, wallet bool) {
	cointype := mycointype // "LTC(XWC)"
...
func testTransfer(t *testing.T, ct string, wallet bool) {
	ms1 := makeheader("xwc_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = ct
	buf1 := doReq(t, ms1)
	fmt.Println("xwc_init response:", string(buf1))
	...
	hex, err := XWCTransaction(xwcreq, "transfer", buf1, cbuf)
	assert.Nil(t, err)

	ms2 := makeheader("xwc_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	// bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
	fmt.Println("broadcast xwc_send data:", string(hex))
	...
	buf2 := doReq(t, ms2)
	fmt.Println("xwc_send response:", string(buf2))
}

func testWithdraw(t *testing.T, wallet bool) {
	ms1 := makeheader("withdraw_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = mycointype //"HC(XWC)"
	buf1 := doReq(t, ms1)
	...
	hex, err := XWCTransaction(xwcreq, "withdraw", buf1, cbuf)
	assert.Nil(t, err)
	ms2 := makeheader("withdraw_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	// bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
...
func testRegister(t *testing.T, wallet bool, name string) {
	ms1 := makeheader("register_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = "XWC"
	buf1 := doReq(t, ms1)
	...
	hex, err := XWCTransaction(xwcreq, "register", buf1, cbuf)
	assert.Nil(t, err)
	ms2 := makeheader("register_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	//bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
...
func testMining(t *testing.T, wallet bool) {
	ms1 := makeheader("mortgage_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = CoinXWC
	// bm["citizenName"] = "a0"
	buf1 := doReq(t, ms1)
	...
	hex, err := XWCTransaction(xwcreq, XWCMining, buf1, cbuf)
	fmt.Println("broadcast mortgage_send data:", string(hex))
	assert.Nil(t, err)
	ms2 := makeheader("mortgage_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	// bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
...
func testRewards(t *testing.T, wallet bool) {
	ms1 := makeheader("income_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = CoinXWC
	buf1 := doReq(t, ms1)
	...
	hex, err := XWCTransaction(xwcreq, XWCRewards, buf1, cbuf)
	assert.Nil(t, err)

	ms2 := makeheader("income_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	// bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
...
func testRedeem(t *testing.T, wallet bool) {
	citizens := mortgageList(t, myxwcaddr)

	ms1 := makeheader("redeem_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	bm[fieldCoinType] = "XWC"
	...
	hex, err := XWCTransaction(xwcreq, XwcRedeem, buf1, cbuf)
	assert.Nil(t, err)

	ms2 := makeheader("redeem_send", xwcwalletid)
	bm2 := ms2["body"].(map[string]interface{})
	bm2["tranContent"] = content
	// bm2["data"] = string(hex)
	setBodyData(t, bm2, hex)
...
func mortgageList(t *testing.T, addr string) []map[string]string {
	ms1 := makeheader("mortgage_list", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
...
func testXWCTransaction(t *testing.T) {
	wallet := true

	xwc.SetTestnetEthSig()
	// testXWCContractInvoke(t, wallet)
	//testBindBTCUSDT(t, wallet)
	//testXWCBind(t, wallet)
	//testUnbind(t, wallet)
...
func testValidateAddress2(t *testing.T) {
	var addres = []string{
		"XWCNZsWKfyfdTiDQhMtqxN2PhfTaLVw9AXutc",
		"XWCNfHB9wQdJx7mhWDXam7gNsvyXd7j1yh1M3",
		"XWCNWjKv1PUbZ6dgVoerkohdoCHRck9LAZh3Y",
		"XWCNL3kJ4prkHUsHsnwW4HGSDUfYxncWcfgDn",
	}

	for _, addr := range addres {
		ok := xwc.ValidateAddress(addr, "testnet")
		assert.True(t, ok)
	}
}

func testConvertXWCSymbol(t *testing.T) {
	vs := []string{
		"Eth", "ETH", "ETH(XWC)", "(XWC)ETH",
	}

	for i, s := range vs {
		ret := convertXWCSymbol(s)
		assert.Equal(t, "ETH", ret, "%s", vs[i])
	}
}

func testXWCContractInvoke(t *testing.T, wallet bool) {
	ct := "XK1"
	ms1 := makeheader("xwc_init", xwcwalletid)
	bm := ms1["body"].(map[string]interface{})
	bm[fieldCoinAddr] = myxwcaddr
	...
	hex, err := XWCTransaction(xwcreq, XWCContractInvoke, buf1, cbuf)
	assert.Nil(t, err)
